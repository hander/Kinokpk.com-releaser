...
$query = "SELECT info_hash,id, completed AS times_completed, seeders, leechers FROM torrents LEFT JOIN xbt_files_users ON torrents.id=fid GROUP BY torrents.id ORDER BY info_hash";
else {
	$hash = bin2hex($_GET["info_hash"]);
	if (mb_strlen($_GET["info_hash"]) != 20)
	err("Invalid info-hash (".mb_strlen($_GET["info_hash"]).")");
	...
	$query = "SELECT torrents.id, xbt_files.info_hash, completed AS times_completed, torrents.seeders, torrents.leechers FROM torrents LEFT JOIN xbt_files ON torrents.id=xbt_files.fid WHERE torrents.info_hash = " . sqlesc($hash)." GROUP BY id";
}
