...
if ($action=='my') {
	...
	$xbt_config_sql = $REL_DB->query("SELECT name,value FROM xbt_config WHERE name IN ('listen_ipa','listen_port','auto_register','redirect_url')");
	...
	while  ($row = mysql_fetch_assoc($r)) {
		...
		if ($REL_CONFIG['use_xbt']) {
			$announce_urls_list[] = ($xbt_config['listen_ipa']=='*'?'http://'.$xbt_config['redirect_url']:"http://{$xbt_config['redirect_url']}")."/".($xbt_config['auto_register']?'':"{$CURUSER['passkey']}/")."announce";
		} else {
...
$rating_enabled = (($REL_CRON['rating_enabled'] && ((time()-$CURUSER['added'])>($REL_CRON['rating_freetime']*86400))
 && ($row['userid']<>$CURUSER['id']) && (!get_privilege('is_vip',false)) && !$userfree && !$row['free'] && !$already_downloaded)?true:false);
...
if ($REL_CONFIG['use_xbt']) { //redirect to bt.sitename, add value to xbt_config->redirect_url, rule for port forwarding bt.sitename:80->bt.sitename:2710
	$announce_urls_list[] = ($xbt_config['listen_ipa']=='*'?'http://'.$xbt_config['redirect_url']:"http://{$xbt_config['redirect_url']}")."/"
	.($xbt_config['auto_register']?'':"{$CURUSER['passkey']}/")."announce";
} else {
	$announce_urls_list[] = $REL_CONFIG['defaultbaseurl']."/announce.php?passkey=".$CURUSER['passkey'];
}
