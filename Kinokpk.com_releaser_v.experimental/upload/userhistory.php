...
if (in_array($type,$allowed_types))
{
	if ($type=='uploaded') {
		$cats = assoc_cats ();
		//xbt fix
		$r = $REL_DB->query("SELECT torrents.id, torrents.name, torrents.added, torrents.seeders, torrents.leechers, torrents.category FROM torrents WHERE owner=$id ORDER BY id DESC");
		if (mysql_num_rows ( $r )) {
	...
	elseif ($type=='downloaded') {
		$cats = assoc_cats ();
		//xbt fix
		//snatched returned back 20110826
		$r = $REL_DB->query ( "SELECT snatched.torrent AS id, snatched.completedat, torrents.name, torrents.seeders, torrents.leechers, torrents.category FROM snatched LEFT JOIN torrents ON torrents.id = snatched.torrent WHERE snatched.userid = $id AND torrents.owner<>$id GROUP BY id ORDER BY id" ) or sqlerr ( __FILE__, __LINE__ );
	...
	elseif ($type=='leeching') {
		...
		$res = $REL_DB->query("SELECT xbt_files_users.fid AS torrent, torrents.added, torrents.name AS torrentname,size, category, torrents.seeders, torrents.leechers FROM xbt_files_users LEFT JOIN torrents ON xbt_files_users.fid = torrents.id WHERE `uid`=$id AND `left`<>0 AND active=1 GROUP BY xbt_files_users.fid");
