...
if ($view == 'leechers') {$condition = "AND p.active = 1 AND p.left<>0"; $countcond = " WHERE x.active = 1 AND x.left<>0";}
if ($view == 'seeders') {$condition = "AND p.active = 1 AND p.left=0"; $countcond = " WHERE x.active = 1 AND x.left=0"; }


$count = @mysql_result(sql_query("SELECT SUM(1) FROM xbt_files_users AS x".$countcond),0);
...
$cheaters = sql_query("SELECT fid AS tid, t.name AS tname, ipa AS ip, port, p.active, p.left, peer_id, p.uid AS userid, u.username, u.class, u.enabled, u.warned, u.donor, u.ratingsum FROM xbt_files_users AS p INNER JOIN users AS u ON u.id = p.uid INNER JOIN torrents AS t ON t.id = p.fid INNER JOIN xbt_announce_log AS x ON x.info_hash = (SELECT info_hash FROM xbt_files WHERE fid=p.fid) WHERE u.enabled = 1 ".$condition." ORDER BY p.mtime DESC $limit") or sqlerr(__FILE__,__LINE__);

$REL_TPL->stdhead("Статистика пиров");

print("<table cellpadding=\"3\" cellspacing=\"0\" width=\"100%\">");
if (get_privilege('view_duplicate_ip'))
...
while ($cheater = mysql_fetch_array($cheaters)) {
	list($tid, $tname, $ip, $port, $seeder, $left, $peer_id, $userid, $username, $class, $enabled, $warned, $donor) = $cheater;
	if ($seeder && $left==0)
